 ...
 */
class SliderList extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
       ...
       // add the fields
       $row = $this->form->addFields( [ new TLabel('Usuario'), $system_user_id ],
                                      [ new TLabel('Status'), $active ],
                                      [ new TLabel('Criado em'), $created_at ],
                                      [ new TLabel('até a data'), $updated_at ]
                                    );
       ...
       $row->layout = ['col-sm-3','col-sm-3','col-sm-3','col-sm-3'];
        ...
        $this->form->addActionLink(_t('New'), new TAction(['SliderForm', 'onEdit']), 'fa:plus green');
        ...
        $column_directory = new TDataGridColumn('directory', 'Imagem', 'left');
        $column_active = new TDataGridColumn('active', 'Status', 'center');
        $column_created_at = new TDataGridColumn('created_at', 'Criado em', 'right');
        $column_updated_at = new TDataGridColumn('updated_at', 'Ediatado', 'right');
        ...
        $this->datagrid->addColumn($column_directory);
        $this->datagrid->addColumn($column_active);
        $this->datagrid->addColumn($column_created_at);
        $this->datagrid->addColumn($column_updated_at);


        $action1 = new TDataGridAction(['SliderForm', 'onEdit'], ['id'=>'{id}']);
    ...
    public function onSearch()
    {
        ...
        TSession::setValue(__CLASS__.'_filter_active',   NULL);
        ...
        if (isset($data->active) AND ($data->active)) {
            $filter = new TFilter('active', 'like', "%{$data->active}%"); // create the filter
            TSession::setValue(__CLASS__.'_filter_active',   $filter); // stores the filter in the session
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for Slider
            $repository = new TRepository('Slider');
            $limit = 10;
            // creates a criteria
            ...
            if (TSession::getValue(__CLASS__.'_filter_active')) {
                $criteria->add(TSession::getValue(__CLASS__.'_filter_active')); // add the session filter
            }
    ...
    public static function Delete($param)
    {
        try
        {
            ...
            TTransaction::open('app'); // open a transaction with database
            $object = new Slider($key, FALSE); // instantiates the Active Record
